...
SOURCE = ROOT / "docs" / "salesforce-backup-restore.md"
OUTPUT = ROOT / "docs" / "salesforce-backup-restore.docx"
...
def add_cover(doc):
    ...
    run = title.add_run("Salesforce Git CI/CD, Backup, Restore, and Product Health")
    ...
    subtitle.add_run("Panduan operasional untuk Trailhead Playground, GitHub Actions, backup metadata, backup data product, monitoring health, dan restore manual.")

    add_simple_table(
        doc,
        ["Area", "Isi"],
        [
            ("Target", "Latihan Git + Salesforce CI/CD menggunakan GitHub Actions"),
            ("Org", "Trailhead Playground atau Developer Edition"),
            ("Repository", "Ogifrn09/salesforce-cicd"),
            ("Output", "Metadata backup, data CSV backup, product health report, dan restore workflow"),
        ],
    )

    note = doc.add_paragraph()
    note.style = "Callout"
    note.add_run("Catatan keamanan: ").bold = True
    note.add_run("repository public tidak cocok untuk backup data production. Gunakan private repository atau secure storage untuk data asli.")
...
def main():
    ...
    for section in doc.sections:
        ...
        if not footer.runs:
            footer.add_run("Salesforce Backup Restore Guide")
